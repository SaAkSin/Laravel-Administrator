...
- **Release:** `13.0.0` (버전 확정, 실제 게시 전)

---

## 🚀 1. 13.0.0 주요 변경

`13.0.0`은 Laravel 13 전용 릴리스로, 설정 기반 디자인 테마와 사용자 정의 CSS/JS 로딩을 추가합니다. 기본 `silver` 테마와 스타일을 추가하지 않는 `legacy` 테마를 제공하며, Vite manifest의 테마 엔트리를 기준으로 배포 에셋을 로드합니다.
...
`13.0.0`은 Laravel `^13.0`과 PHP `^8.3`만 지원합니다.

---

## 🛠️ 3. 설치 방법 (Composer)

Composer를 사용하여 Laravel 13 애플리케이션에 설치할 수 있습니다. 다음 명령은 `13.0.0`이 게시된 뒤 사용합니다.
...
`13.0.0`은 Laravel `^13.0`과 PHP `^8.3`만 지원하며, 이 범위를 벗어난 런타임은 지원하지 않습니다.
...
변경 이력은 [changelog.md](changelog.md)를 기준으로 관리합니다. `13.0.0`의 배포 요약과 승인 체크리스트는 [RELEASE_NOTES.md](RELEASE_NOTES.md)에서 확인할 수 있습니다.
