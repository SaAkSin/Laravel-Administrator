...
기존 FrozenNode의 `Laravel-Administrator` 패키지가 더 이상 업데이트되지 않아, 이를 바탕으로 Laravel 10~13 및 최신 프론트엔드 아키텍처 환경에 맞춰 개발하고 있는 관리자 페이지 빌더 패키지입니다.
...
- **Release candidate:** `10.8.0` (최종 태그명 미정)

---

## 🚀 1. 10.8.0 릴리스 후보 주요 변경

`10.8.0` 릴리스 후보는 Laravel 10~13 호환 범위를 명시하고, 설정 기반 디자인 테마와 사용자 정의 CSS/JS 로딩을 추가합니다. 기본 `silver` 테마와 스타일을 추가하지 않는 `legacy` 테마를 제공하며, Vite manifest의 테마 엔트리를 기준으로 배포 에셋을 로드합니다.
...
| Laravel | PHP | 테스트 도구 계열 |
| --- | --- | --- |
| 10.x | 8.1 이상 | Testbench 8 / PHPUnit 9.6 |
| 11.x | 8.2 이상 | Testbench 9 / PHPUnit 10.5 |
| 12.x | 8.2 이상 | Testbench 10 / PHPUnit 11 |
| 13.x | 8.3 이상 | Testbench 11 / PHPUnit 11 |

패키지의 Composer 제약은 PHP `>=8.1`, Laravel `^10.0|^11.0|^12.0|^13.0`입니다. 각 Laravel 버전이 요구하는 최소 PHP 버전도 함께 충족해야 합니다.

> [!WARNING]
> 2026-07-15 릴리스 후보 검증에서는 Composer의 기본 보안 차단 정책이 Laravel 10·11 의존성 해석을 거부했습니다. 지원 범위는 유지하지만, 실제 배포 전에 관련 advisory가 해소된 안전한 조합을 다시 확인해야 합니다. 운영 설치에서 `--no-security-blocking`을 사용하지 마십시오.

---

## 🛠️ 3. 설치 방법 (Composer)

Composer를 사용하여 지원되는 Laravel 애플리케이션에 설치할 수 있습니다. 다음 명령은 `10.8.0` 태그가 확정·배포된 뒤 사용합니다.

```sh
composer require "saaksin/laravel-administrator:^10.8"
```
...
`10.8.0` 릴리스 후보의 지원 범위는 Laravel 10.x~13.x입니다. Laravel 9 이하는 지원하지 않습니다. Laravel 주 버전을 올릴 때는 위 호환성 표에 맞춰 PHP, Testbench, PHPUnit 조합을 함께 조정하십시오.
...
변경 이력은 [changelog.md](changelog.md)를 기준으로 관리합니다. `10.8.0` 릴리스 후보의 배포 요약과 승인 체크리스트는 [RELEASE_NOTES.md](RELEASE_NOTES.md)에서 확인할 수 있습니다.
