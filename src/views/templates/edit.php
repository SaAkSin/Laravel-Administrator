					...
					<template x-if="$root[field.field_name] && !loadingItem">
						<div class="image_container" style="position: relative; display: block; width: 100%; margin-top: 8px; box-sizing: border-box;">
							<template x-if="field.display_raw_value">
								<img :src="$root[field.field_name]" style="max-width: 100px; height: auto; border: 1px solid #ccc; padding: 2px; display: inline-block; vertical-align: top;" />
							</template>
							<template x-if="!field.display_raw_value">
								<img :src="$root.file_url + '?path=' + field.location + $root[field.field_name]" style="max-width: 100px; height: auto; border: 1px solid #ccc; padding: 2px; display: inline-block; vertical-align: top;" />
							</template>
							<template x-if="field.editable">
