...
- 마크다운 링크와 이미지는 프로토콜 검증만으로 충분하지 않다. `href`, `src`, `title`, `alt` 등 HTML attribute context에 삽입되는 값은 반드시 escape 처리해야 한다.
- URL 검증은 입력 원문이 아니라 HTML entity decode, 제어문자/공백 제거, scheme 정규화 이후의 값을 기준으로 수행한다.
- 허용 프로토콜은 allowlist 방식으로 제한한다. 링크는 `http:`, `https:`, `mailto:`, `tel:` 및 상대 URL을 허용하고, 이미지는 `http:`, `https:` 및 상대 URL만 허용하는 것을 기본 정책으로 한다.
- `javascript:`, `data:`, `vbscript:`, entity-encoded protocol, 따옴표 기반 attribute injection은 모두 차단되어야 한다.
- 파일/에셋 경로 접근 보강 로직은 유지한다.
- 인증/인가 우회가 발생하지 않도록 관리자 미들웨어와 모델별 permission 흐름을 건드릴 때는 회귀 테스트를 추가한다.
- 민감정보나 토큰이 로그에 출력되지 않도록 한다.

### 마크다운 sanitizer 보완 예제

아래 코드는 구현 방향을 명확히 하기 위한 예시다. 실제 반영 시에는 기존 `resources/js/app.ts` 구조에 맞게 함수명을 조정하고, 가능하면 helper를 분리해 회귀 검증이 가능하도록 작성한다.
...
검증된 sanitizer 패키지 도입이 가능하다면 DOMPurify 또는 sanitize-html 같은 라이브러리 사용을 우선 검토한다. 직접 구현을 유지하는 경우 위 예시와 동등한 수준으로 canonicalization, allowlist, attribute escaping을 모두 포함해야 한다.
...
- 마크다운 sanitizer 변경 시 다음 입력은 최소 회귀 검증에 포함한다.
...
마크다운 회귀 검증 예시는 다음과 같다. 테스트 러너가 없으면 동일한 케이스를 브라우저 `DOMParser` 또는 jsdom 기반 smoke script로 실행하고 결과보고서에 출력 결과를 기록한다. 단순 문자열 검사는 escape된 텍스트까지 오탐할 수 있으므로 실제 DOM attribute를 기준으로 판단한다.
...
- [ ] 마크다운 링크/이미지 sanitizer가 attribute injection과 entity-encoded dangerous protocol을 모두 차단한다.
- [ ] 마크다운 XSS 회귀 검증 입력과 출력 결과가 결과보고서 또는 테스트 코드에 기록된다.
...
- 미실행 테스트와 사유
- 보안 검토 결과
- 마크다운 sanitizer 회귀 검증 결과
- 문서 정합성 확인 결과
- 남은 리스크와 후속 이슈 제안
- 실제 원격 브랜치 HEAD와 일치하는 커밋 SHA와 브랜치
