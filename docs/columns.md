...
* [Relationships](#relationship-columns) (inlcuding [nested relationships](./relationship-columns.md#nested-relationships))
...
> For a more in-depth look at relationship columns, check out the [relationship columns docs](./relationship-columns.md)
...
Any SQL grouping function will work in the `select` statement. Much more is possible with the `relationship` option, so check out the [relationship columns docs](./relationship-columns.md) for more detail.
...
If you provide an anonymous function, the argumentis available are the relevant column's value from the database, and the current model. For instance, if you are using a [color](./field-type-color.md) field and you want to clearly show the admin user what color a row is, you can do this:
...
Or maybe you're using an [image](./field-type-image.md) field and you want to display the image in your result set. In that case you'd do:
