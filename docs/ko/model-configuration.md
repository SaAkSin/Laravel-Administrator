...
> **💡 팁 - 설정 파일의 현대적인 리팩토링 권장**: 
> 과거 레거시 버전에서는 글로벌 익명 함수(예: `function 모델명() { return array(...); }`)를 선언하여 호출하는 방식이 사용되었으나, 이 방식은 라라벨 설정 캐싱(`php artisan config:cache`) 수행 시 중복 함수 선언 충돌이나 런타임 함수 없음 오류를 유발할 수 있습니다. 
> 따라서, 최신 버전에서는 글로벌 함수 선언을 지양하고 파일 최상단에서 곧바로 `return [ ... ];` 형태의 **순수 PHP 배열을 반환**하는 방식으로 설계하실 것을 강력히 권장합니다.
